...
export class SubjectService {
  ...
  protected convertDateFromClient(subject: ISubject): ISubject {
    return Object.assign({}, subject, {
      createDate: subject.createDate?.isValid() ? subject.createDate.toJSON() : undefined,
      updateDate: subject.updateDate?.isValid() ? subject.updateDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((subject: ISubject) => {
        subject.createDate = subject.createDate ? dayjs(subject.createDate) : undefined;
        subject.updateDate = subject.updateDate ? dayjs(subject.updateDate) : undefined;
      });
    }
    return res;
  }
